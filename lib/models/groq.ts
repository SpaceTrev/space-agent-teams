...
// ============================================================
// Main callGroqModel function
// ============================================================

export async function callGroqModel(options: ModelCallOptions): Promise<ModelCallResult> {
  const apiKey = options.api_key ?? process.env.GROQ_API_KEY
  if (!apiKey) {
    throw new Error('Groq API key is required. Set GROQ_API_KEY or pass api_key.')
  }

  const client = new OpenAI({
    apiKey,
    baseURL: options.base_url ?? 'https://api.groq.com/openai/v1',
  })
